...
desc('Set file protection');
task('deploy:protect', function () {
  run("find {{release_path}} -type d -print0 | sudo xargs -0 chmod 2754");
  run("find {{release_path}} -type f -print0 | sudo xargs -0 chmod 0644");
  run("sudo chmod 2775 {{release_path}}/joomla/templates/rt_orion/custom/css-compiled");
  run("find {{release_path}}/joomla/templates/rt_orion/custom/css-compiled -type f -print0 | sudo xargs -0 chmod 0664");
});
