...
*/
class JFormFieldExporter extends SigPlusNovoFormField {
	...
	public function getInput() {
		...
		if (file_exists($scriptpath.DIRECTORY_SEPARATOR.'js'.DIRECTORY_SEPARATOR.'exporter.min.js') || file_exists($scriptpath.DIRECTORY_SEPARATOR.'js'.DIRECTORY_SEPARATOR.'exporter.js')) {
			// get identifiers
			$class = ( isset($this->element['class']) ? (string)$this->element['class'] : 'form-control' );
			...
			$html .=
				'<button type="button" id="'.$exportctrlid.'">'.Text::_('SIGPLUS_SETTINGS_EXPORT').'</button>'.
				'<button type="button" id="'.$importctrlid.'">'.Text::_('SIGPLUS_SETTINGS_IMPORT').'</button>'.
			...
			if (file_exists($scriptpath.DIRECTORY_SEPARATOR.'js'.DIRECTORY_SEPARATOR.'exporter.min.js') && filemtime($scriptpath.DIRECTORY_SEPARATOR.'js'.DIRECTORY_SEPARATOR.'exporter.min.js') >= filemtime($scriptpath.DIRECTORY_SEPARATOR.'js'.DIRECTORY_SEPARATOR.'exporter.js')) {
				$document->addScript(Uri::root(true).'/plugins/content/'.SIGPLUS_PLUGIN_FOLDER.'/fields/js/exporter.min.js');
			} else {
				$document->addScript(Uri::root(true).'/plugins/content/'.SIGPLUS_PLUGIN_FOLDER.'/fields/js/exporter.js');
			}
