...
abstract class SigPlusNovoAtomFeedGallery extends SigPlusNovoXMLGallery {
	protected function requestFolder($feedurl, &$folderparams, $url, $viewid) {
		...
		if ($sxml === false) {
			SigPlusNovoLogging::appendError('Unable to retrieve Atom feed for <code>'.htmlspecialchars($url).'</code>; cannot parse response as XML.');
			throw new SigPlusNovoXMLFeedException($url);
		}
...
class SigPlusNovoFlickrGallery extends SigPlusNovoXMLGallery {
	...
	*/
	public function populate($url, $folderparams) {
		...
		$sxml = simplexml_load_string($feeddata);
		if ($sxml === false) {
			SigPlusNovoLogging::appendError('Unable to retrieve Flickr Web Album with user ID <code>'.htmlspecialchars($user_id).'</code> and photo set ID <code>'.htmlspecialchars($photoset_id).'</code>; cannot parse response as XML.');
			throw new SigPlusNovoXMLFeedException($url);
		}
		if ($sxml->err) {
		...
		$feedquery = array(
			...
			'user_id' => $user_id,
			'photoset_id' => $photoset_id,
			'extras' => 'last_update,o_dims,url_o,description',
			'format' => 'rest'
		);
		$feedurl = 'https://api.flickr.com/services/rest/?'.http_build_query($feedquery);
		...
		$sxml = simplexml_load_string($feeddata);
		if ($sxml === false) {
			SigPlusNovoLogging::appendError('Unable to retrieve Flickr Web Album photos with user ID <code>'.htmlspecialchars($user_id).'</code> and photo set ID <code>'.htmlspecialchars($photoset_id).'</code>; cannot parse response as XML.');
			throw new SigPlusNovoXMLFeedException($url);
		}
		if ($sxml->err) {
			SigPlusNovoLogging::appendError('Unable to retrieve Flickr Web Album photos with user ID <code>'.htmlspecialchars($user_id).'</code> and photo set ID <code>'.htmlspecialchars($photoset_id).'</code>; query to Flickr API fails with code <code>'.htmlspecialchars($sxml->err['code']).'</code> and message: <em>'.htmlspecialchars($sxml->err['msg']).'</em>');
			throw new SigPlusNovoXMLFeedException($url);
		}
		...
		if (isset($sxml->photoset) && isset($sxml->photoset->photo)) {
			foreach ($sxml->photoset->photo as $photo) {  // enumerate album entries with XPath "/photoset/photo"
				...
				if (empty($image_url)) {
					SigPlusNovoLogging::appendError('The photo with ID <code>'.$id.'</code> in the Flickr Web Album with user ID <code>'.htmlspecialchars($user_id).'</code> and photo set ID <code>'.htmlspecialchars($photoset_id).'</code> has no URL. Do you have the right permissions in Flickr to access this image (from outside of Flickr)?');
					throw new SigPlusNovoXMLFeedException($url);
				}
		...
		// update caption data
		if (!empty($captions)) {
			$this->populateCaptions($folderparams->id, $captions);
		}
	...
	private function populateCaptions($folderid, $captions) {
		...
		foreach ($captions as $index => $caption) {
			...
			$row = array(
				...
				$countryid,
				$index,
				$db->quote($caption->title),
				$db->quote($caption->description)
			);
			$rows[] = '('.implode(',',$row).')';
		}
				...
				$titlequery =
					'COALESCE('.PHP_EOL.
						...
						'('.PHP_EOL.
							...
							$limit1.PHP_EOL.
						'),'.PHP_EOL.
						// or use meta-data field "Object Name" (a.k.a. document title)
						'('.PHP_EOL.
							'SELECT '.$top1.' md.'.$db->quoteName('textvalue').''.PHP_EOL.
							'FROM '.$db->quoteName('#__sigplus_property').' AS mp'.PHP_EOL.
							'INNER JOIN '.$db->quoteName('#__sigplus_data').' AS md'.PHP_EOL.
							'ON mp.'.$db->quoteName('propertyid').' = md.'.$db->quoteName('propertyid').PHP_EOL.
							'WHERE mp.'.$db->quoteName('propertyname').' = '.$db->quote('Object Name').' AND md.'.$db->quoteName('imageid').' = i.'.$db->quoteName('imageid').''.PHP_EOL.
							$limit1.PHP_EOL.
						'),'.PHP_EOL.
