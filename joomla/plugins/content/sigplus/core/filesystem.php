...
class fsx_windows implements fsx_functions {
	...
	*/
	...
	private static function is_ascii($string) {
		return empty($string) || preg_match('/[\\x80-\\xff]+/', $string) === 0;
	}
...
function safe_url_encode($url) {
	...
	foreach ($segments as &$segment) {
		if (preg_match($pattern, $segment) === 0) {  // path segment contains a character that has not been URL-encoded
			$segment = rawurlencode($segment);
		}
	}
	$urlparts['path'] = implode('/', $segments);
	if (!empty($urlparts['query'])) {
		if (preg_match($pattern, $urlparts['query']) === 0) {  // query contains a character that has not been URL-encoded
...
function path_url_encode($path) {
	...
	if (preg_match('#^[A-Za-z0-9]:$#S', $parts[0]) === 1) {  // a Windows-style path with drive letter
...
function is_url_http($string) {
	return !empty($string) && preg_match('#^https?://#', $string) === 1;
}
...
function is_absolute_path($path) {
	return !empty($path) && preg_match('#^(/|([A-Za-z0-9]:)?\\\\)#S', $path) === 1;
}
