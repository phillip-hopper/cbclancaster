...
class SigPlusNovoBoxPlusLightboxEngine extends SigPlusNovoLightboxEngine {
	...
	*/
	public function addStyles($selector, SigPlusNovoGalleryParameters $params) {
		...
		if (count($parts) > 1) {
			$theme = $parts[1];
			$selectorMain = "{$prefix}.boxplusx-dialog, {$prefix}.boxplusx-detail";
			$selectorData = "{$prefix}.boxplusx-detail td";
			switch ($theme) {
		...
		$css[$prefix.'.boxplusx-navigation'] = $navigation;

		$css[$prefix.'.boxplusx-navitem'] = array('width' => $params->thumb_width.'px');
		...
		$css[$prefix.'.boxplusx-dialog.boxplusx-animation'] = array('transition-timing-function' => $easing);
