...
class plgSearchSigPlusNovo extends JPlugin {
	...
	*/
	public function onContentSearch($text, $phrase = '', $ordering = '', $areas = null) {
		...
		// build query to fetch caption and longer description text
		$titlequery =
			'COALESCE('.PHP_EOL.
		...
		if ($rows) {
			...
			foreach ($rows as $row) {
				...
				$results[] = (object) array(
					...
					// standard Joomla search code strips HTML tags from search result text and imposes a length limit
					'title'       => html_entity_decode(strip_tags($title), ENT_QUOTES, 'utf-8'),
					'text'        => html_entity_decode(strip_tags($summary), ENT_QUOTES, 'utf-8'),

					// custom fields are included to pass full title/text to templates that override the search results page
					'caption'     => $title,
					'description' => $summary
				);
				...
				if ($show_thumbnails) {
					...
					$instance->addOnReadyScript("__sigplusSearch({$target_url}, {$preview_url}, {$width}, {$height}, {$title}, {$summary});");
				}
			}
